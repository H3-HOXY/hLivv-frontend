...
export function MyRestoreModal({productDto, isActive, onClickModalOff}: {
    ...
    const onClickCardConfirm = () => {
        submitForm(productDto.id!!, restoreDesc!!.toString(), restoreGrade!! as "S" | "A" | "B", false, [], imgFile).then(
            () => {
                // 모달을 닫고 캐시 비우기
                onClickModalOff();
                ...
                <div className="RestoreModal">
                    ...
                    <div className="RestoreModalInfo">
                        ...
                        <div className="RestoreModalInfoContent">조건에 맞지 않는 제품은 리스토어 판매가 어렵습니다.
                            <br/>H.Livv 리스토어 서비스는 가구에 제2의 삶을 불어 넣을 수 있는 선택 중 하나일 뿐입니다. 가구를 폐기할 때가 되었다면 다른 재활용 방법을
                            고려해보세요.
                        </div>
                    </div>
                </div>
...
async function submitForm(productId: number, restoreDesc: string, requestGrade: "S" | "A" | "B" | "C", whenRejected: boolean, restoreImageUrls: string[], file: File | undefined) {
    const imgFile = file !== undefined ? {imageFile: file} : {}
    try {
        const api = await getApi();
        await api.restoreRegister1({
            productId: productId,
